...
function install() {
	echo -e " Installing xploitoverload Helium.. "
	...
	wget -q -O /etc/dnsmasq.conf "https://raw.githubusercontent.com/xploitoverload/helium/main/dnsmasq.conf"
	sed -i "s/YourPublicIP/${publicIP}/" /etc/dnsmasq.conf
	rm -rf ${providers}
	wget -q -O ${providers} "https://raw.githubusercontent.com/xploitoverload/helium/main/providers.txt"
	rm -rf /usr/local/sbin/helium_daily
	wget -q -O /usr/local/sbin/helium_daily "https://raw.githubusercontent.com/xploitoverload/helium/main/helium_daily.sh"
	chmod 755 /usr/local/sbin/helium_daily
	sed -i '/helium_daily/d' /etc/crontab
	echo -e "0 4 * * * root helium_daily # Helium by xploitoverload" >>/etc/crontab
	updateEngine
	...
	>/etc/resolvconf/resolv.conf.d/head
        ...
        header
        echo
	echo -e " Installation completed by xploitoverload"
	sleep 1
...
function start() {
	...
	echo -e -n " Starting xploitoverload Helium..."
...
function DNSOption() {
	...
	echo
        heliumStatus
        echo
	echo -e " ${WHITE}Change xploitoverload DNS${NOCOLOR}"
	echo -e " [1] Google
 [2] Cloudflare
...
function changeDNS() {
	echo -e -n " Changing to ${PROVIDER} xploitoverload DNS..."
...
function reinstall() {
	...
	echo -e -n " Reinstalling xploitoverload Helium..."
	sleep 2
	[[ ! -e /etc/dnsmasq ]] && mkdir -p /etc/dnsmasq
	echo "nameserver 1.1.1.1" >/etc/resolv.conf
	echo
...
function uninstall() {
	...
	echo -e -n " Uninstalling xploitoverload Helium..."
...
function cleaner() {
	...
	#wget -q -O /etc/dnsmasq/dead.hosts "https://raw.githubusercontent.com/xploitoverload/helium/main/dead.hosts"
...
function updateHelium() {
	...
	wget -q -O /tmp/helium.tmp "https://raw.githubusercontent.com/xploitoverload/helium/main/helium.sh"
	...
	echo -n -e " Updating xploitoverload Helium..."
	if [[ ${OVERWRITE,,} == "y" ]]; then
		rm -rf /etc/dnsmasq/providers.txt
		wget -q -O /etc/dnsmasq/providers.txt "https://raw.githubusercontent.com/xploitoverload/helium/main/providers.txt"
	fi
